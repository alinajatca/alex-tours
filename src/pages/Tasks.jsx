...
import { Plus, CheckCircle2, Circle, ArrowUpCircle, Trash2, X } from "lucide-react";
...
export default function Tasks() {
  ...
  const handleCreate = (e) => {
    e.preventDefault();
    createMutation.mutate({ ...form, created_by_name: user?.email });
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <p className="text-sm text-slate-500">
          {user?.isManager ? "Atribuie și urmărește sarcinile echipei" : "Sarcinile tale și ale echipei"}
        </p>
        {user?.isManager && (
        ...
        {showForm && user?.isManager && (
          <Motion.form initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <div className="md:col-span-2 flex justify-end gap-3">
              ...
              <Button type="submit" style={{ backgroundColor: "#00b5b5" }} className="text-white">Creează sarcină</Button>
            </div>
          </Motion.form>
        )}
      </AnimatePresence>
